...
from ..core.game_rules import GameRules, compile_rules, normalize_rules, source_ir
from ..core.interpreter import Interpreter
from ..core.ir import REQUIRED_AXES, check_ir, required_axes
...
TOOL_SCHEMAS: tuple[dict[str, Any], ...] = (
    ...
    {"name": "compose_plan", "args": {},
     "description": "由宿主通过 GameRules 1.0 确定性编译候选计划。"},
...
def dispatch(tool: str, args: dict[str, Any], state: LoopState,
    ...
    if tool == "propose_ir":
        ...
        try:
            state.ir = normalize_rules(payload, registry)
        ...
        report = check_ir(source)
        return _ok(tool, kind=state.ir.kind, source_kind=source.kind,
                   required_axes=required_axes(source),
    ...
    if tool == "patch_ir":
        ...
        merged = state.ir.model_dump(mode="json")
        merged["execution"]["rules"].update(values)
        try:
            state.ir = normalize_rules(merged, registry)
    ...
    if tool == "compose_plan":
        ...
        try:
            if provided is not None:
                return _fail(tool, "raw_plan_not_accepted")
            if state.ir is None:
                return _fail(tool, "propose_ir_first")
            plan = compile_rules(state.ir, registry).plan
