...
AXES: tuple[Capability, ...] = (
    ...
    Capability("pattern_lang", "参数化牌型与合法性", "planned",
               ("axis.pattern_lang",)),
    Capability("rank_compare", "通用牌力比较（不内置玩法牌型）", "planned",
               ("axis.rank_compare",)),
    Capability("info_set", "信息集与按视角可见性", "planned",
               ("axis.info_set", "hidden_position_draw")),
    Capability("turn_adapter", "优先级 / 墩牌 / 竞叫 / 同时行动", "planned",
               ("axis.turn_adapter",)),
    Capability("trigger", "通用触发器与效果", "planned",
               ("axis.trigger",)),
    Capability("team", "队伍与合作胜负", "planned",
               ("axis.team",)),
