...
def core_registry() -> ToolRegistry:
    ...
    registry.register(ToolSpec("zones", lambda **_: ZonesTool(), (
        ...
        OperationSpec("count", params=("state", "zone"), effects=(), returns="counts",
                      reads=("zones",),
                      input_schema=obj({"state": OBJECT, "zone": STRING}, ("state",)),
                      output_schema=obj({"counts": OBJECT, "count": INTEGER}),
                      feature_constraints=("card_identity", "multi_zone")),
